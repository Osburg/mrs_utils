...
class Axis:
    ...
    def to_index(self, value: float, domain: str = "time") -> int:
        ...
        if domain == "time":
            if value < np.min(self._time) or value > np.max(self._time):
                warnings.warn("The value is outside the covered time interval. The closest value will be used.")
            return np.argmin(np.abs(self._time - value))
        if domain == "frequency":
            if value < np.min(self._frequency) or value > np.max(self._frequency):
                warnings.warn("The value is outside the covered frequency interval. The closest value will be used.")
    ...
    def from_time_axis(cls, time: np.ndarray, b0: Optional[float] = 3.) -> Self:
        ...
        obj._f0 = GAMMA_H1 * b0
        obj._time = time
        obj._frequency = obj._f0 + fftshift(fftfreq(obj.n, obj._time[1] - obj._time[0])) * 1e-6
        obj._ppm = np.flip((obj._frequency - obj._f0) / obj._f0 * 1e6 + obj.cs0)
        return obj

    @classmethod
    def from_frequency_axis(cls, frequency: np.ndarray, b0: Optional[float] = 3.) -> Self:
        ...
        obj._time = np.arange(len(frequency)) / ( obj.frequency_spacing * len(frequency) * 1e6 )
        obj._ppm = np.flip((obj._frequency - obj._f0) / obj._f0 * 1e6 + obj.cs0)
        return obj

    @classmethod
    def from_ppm_axis(cls, ppm: np.ndarray, b0: Optional[float] = 3.) -> Self:
        ...
        obj._time = np.arange(len(obj._frequency)) / (obj.frequency_spacing * len(obj._frequency) * 1e6)
        return obj